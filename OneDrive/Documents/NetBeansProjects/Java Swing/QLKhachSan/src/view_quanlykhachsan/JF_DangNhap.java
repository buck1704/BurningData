...
public class JF_DangNhap extends javax.swing.JFrame {
    ...
    private void bt_DangNhapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bt_DangNhapActionPerformed
        try {
            Connection conn = DriverManager.getConnection(QLKhachSan.dbURL);
            String UN, FN, PW;
            UN = txt_TaiKhoan.getText();
            PW = txt_MatKhau.getText();
            String sql = "SELECT taikhoan, matkhau, manhanvien From taikhoan where taikhoan = '"+ UN +"' and matkhau = '"+ PW +"'";
            Statement state = conn.createStatement();
            ResultSet rs = state.executeQuery(sql);
            if (rs.next()) {
                QLKhachSan.sTenDN = rs.getString("taikhoan");
                QLKhachSan.sMatKhau = rs.getString("matkhau");
                QLKhachSan.sMaNhanVien = rs.getString("manhanvien");
                JP_PhieuDangKy.laymanhanvien = QLKhachSan.sMaNhanVien;
                dispose();
                System.out.println("Notification: Đăng Nhập Thành Công");
